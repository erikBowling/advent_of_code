...
const DAY04_DIRECTIONS: [(i32, i32); 8] = [
    (0, 1), // down
    (0, -1), // up
    (1, 0), // right
    (-1, 0), //left
...
impl Day04 {
    fn get_num_adjacent(&self, bounds: (usize, usize), pos: (usize, usize), grid: &Vec<Vec<char>> ) -> usize{
        let mut num_adjacent: usize = 0;
        for dir in DAY04_DIRECTIONS{
            let check_row: i32 = pos.0 as i32 + dir.0;
            let check_col: i32 = pos.1 as i32 + dir.1;

            // row
            match check_row {
                n if n < 0 => { continue; },
                n if n > bounds.0 as i32 => { continue; },
                _ => {}
            }
            ...
            match check_col {
                n if n < 0 => { continue; },
                n if n > bounds.1 as i32 => { continue; },
                _ => {}
            }

            if grid[check_row as usize][check_col as usize] == '@' { num_adjacent += 1 };
        }
